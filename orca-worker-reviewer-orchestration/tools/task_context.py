...
def phase_artifact_contract(*, role: str, phase: str, run_id: str = "",
                            gate_iteration: int = 1) -> str:
    ...
    # OS-42: the path ladder lives in ONE place -- the engine package, which is the only
    # module every consumer (this one, e2e_harness, run_logging, review_isolation) can
    # reach because it is the one that ships. `gate_iteration` is last and defaults to 1,
    # so every pre-existing call binds unchanged and resolves to the unsuffixed name.
    try:
        from scripts.deterministic_workflow.artifact_identity import (
            ArtifactIdentityError, artifact_basename)
        ...
        from deterministic_workflow.artifact_identity import (  # type: ignore[no-redef]
            ArtifactIdentityError, artifact_basename)
    # This module keeps its OWN run_artifact_root, so its run-id contract -- required,
    # and refused when it contains a path separator or a `.`/`..` segment that could
    # climb out of artifacts/runs/ -- is unchanged and is still checked here.
    base = run_artifact_root(run_id)
    try:
        return base + artifact_basename(phase=phase, role=role,
...
# OS-42. The generated decision-gate contract and, on a repair dispatch only, the
# structured validation error that caused it. Both are rendered elsewhere
# (`decision_contract`) and only carried here, so this module holds no schema knowledge.
DECISION_GATE_SPEC_HEADER = "=== DECISION GATE CONTRACT (generated) ==="
DECISION_GATE_SPEC_FOOTER = "=== END DECISION GATE CONTRACT ==="
VALIDATION_REPAIR_SPEC_HEADER = "=== VALIDATION REPAIR ==="
VALIDATION_REPAIR_SPEC_FOOTER = "=== END VALIDATION REPAIR ==="
...
def render_task_spec(
    ...
    decision_gate_contract: str | None = None,
    validation_repair: str | None = None,
) -> str:
    ...
    if decision_gate_contract is not None:
        # OS-42. Appended after the agent-routing block and before the end marker, exactly
        # as the risk and agent-routing blocks were added, so a caller that omits it
        # renders a byte-identical spec to before this parameter existed. The block is
        # generated from the live schema; nothing here transcribes a value.
        lines.append("")
        lines.append(decision_gate_contract)
    if validation_repair is not None:
        # Present ONLY on a repair dispatch. It carries the validator's own error, the
        # field path, what the agent sent and the COMPLETE allowed set -- never a chosen
        # value.
        lines.append("")
        lines.append(validation_repair)
