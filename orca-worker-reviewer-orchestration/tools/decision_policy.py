...
def domain_defect(spec: BoundaryElement, value: Any) -> str | None:
    """Public alias for :func:`_domain_defect`, added by OS-42.

    `decision_gate.collect_form_defects` needs the contract's OWN domain rule so the
    FORM sweep is derived from the declared schema rather than restating it. A one-line
    re-export is the smallest change that gives it that: no behaviour moves, the private
    name stays the implementation, and this module keeps its contract-only isolation.
    """
    return _domain_defect(spec, value)
