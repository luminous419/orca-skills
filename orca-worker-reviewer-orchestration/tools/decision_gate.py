...
class GateRefusal(DecisionGateError):
    ...
    def __init__(
        self, reason: str, detail: str = "", defects: "tuple[GateDefect, ...]" = ()
    ) -> None:
        super().__init__(f"{reason}{(' -- ' + detail) if detail else ''}")
        self.reason = reason
        self.detail = detail
        # OS-42. Purely additive and defaulted: the message string, `reason` and `detail`
        # are byte-identical to before, so every existing `except GateRefusal` site and
        # every test asserting on `.reason` is unaffected. `detail` is free text and
        # cannot carry a field path, an allowed set or an offending value in a form a
        # terminal reason can record -- that is what `defects` is for.
        self.defects = tuple(defects)


# ---- OS-42: the defect taxonomy and its structured payload ------------------------
# `DECISION_GATE_INPUT_MALFORMED` is one code covering three unrelated things: a JSON
# fence that will not parse, an enum token outside its closed set, and an
# ASSUMPTION_ALLOWED that INV-4 forbids. The first two are representation defects a
# bounded re-ask can fix; the third is a judgement and re-asking for it is laundering.
# The taxonomy below is what separates them, and its default arm is what keeps the
# separation safe when someone adds a reason code and forgets this file.
DEFECT_KINDS: tuple[str, ...] = ("FORM", "SEMANTIC", "LIFECYCLE")
...
# The offending value is echoed back to the agent, so it is bounded here rather than at
# the render site: an unbounded `actual` would put a whole malformed body into a
# checkpoint and into a terminal reason.
ACTUAL_MAX = 200

FORM_DEFECT_CODES: frozenset[str] = frozenset(
    {GATE_INPUT_MISSING, GATE_INPUT_MALFORMED, SUMMARY_DISAGREES_WITH_RECORD}
)
SEMANTIC_DEFECT_CODES: frozenset[str] = frozenset({DOWNGRADE_REJECTED})
LIFECYCLE_DEFECT_CODES: frozenset[str] = frozenset(
    {
        LEDGER_SCHEMA_UNSUPPORTED,
        LEDGER_INCONSISTENT,
        GATE_INPUT_UNBOUND,
...
def truncate_actual(value: Any) -> str:
    """`repr` of the offending value, bounded. Never the value itself."""
    text = repr(value)
    return text if len(text) <= ACTUAL_MAX else text[: ACTUAL_MAX - 1] + "\u2026"


@dataclass(frozen=True)
class GateDefect:
    """One machine-readable defect: what was wrong, where, and what was allowed.

    `GateRefusal.detail` is free text and cannot answer "which field", "which values"
    or "what did the agent actually send" in a form a terminal reason can record. This
    record can, which is why the ticket's terminal-reason requirement -- validation
    error, field path, allowed values, retry count -- is met by a list of these plus a
    counter rather than by a sentence.

    There is deliberately NO `suggestion`, `candidate` or `recommended` field. The key
    set is closed and pinned by a test, so there is nowhere for an inferred value to
    travel from the validator to the agent. `expected` carries the WHOLE allowed set,
    ...
    def __post_init__(self) -> None:
        if self.kind not in DEFECT_KINDS:
            raise DecisionGateError(f"unknown defect kind {self.kind!r}")

    def as_dict(self) -> dict[str, Any]:
        """Exactly DEFECT_KEYS, in JSON-native types.

        The list is written into WorkflowState, so every value has to survive
        `state._checkpointable`, which admits only dict/list/bool/int/str/None --
        hence `expected` as a list rather than a tuple.
        """
        return {
            "code": self.code,
            "kind": self.kind,
...
def defect_kind(code: str) -> str:
    """FAIL-CLOSED. Returns "FORM" only for a positively enumerated FORM code.

    The final arm is the whole safety property: a code nobody classified is LIFECYCLE,
    and LIFECYCLE is never repaired. A refusal reason added to GATE_REFUSAL_REASONS
    without an entry here therefore BLOCKS rather than being retried, and
    `test_every_refusal_reason_has_an_explicit_kind` turns that silent safety into a
    loud one.

    Membership in FORM_DEFECT_CODES never grants repairability by itself:
    `classify_gate` constructs GateDefect objects with `kind` already set at the point
    of detection, because GATE_INPUT_MALFORMED is raised by both form and judgement
    checks. This function is the fallback for a code arriving from elsewhere -- a
    GateRefusal raised by an existing caller -- not the primary path.
    """
    if BLOCK_REASON_PATTERN.match(code) is not None:
        return "SEMANTIC"
    if code in SEMANTIC_DEFECT_CODES:
        return "SEMANTIC"
    if code in LIFECYCLE_DEFECT_CODES:
        return "LIFECYCLE"
    if code in FORM_DEFECT_CODES:
        return "FORM"
    return "LIFECYCLE"


def _form(code: str, field_path: str, expected: Sequence[str], actual: Any,
          message: str) -> GateDefect:
    return GateDefect(
        code=code,
        kind="FORM",
...
# The JSON type the LEDGER schema fixes for each record field it owns. The decision
# policy declares domains for the boundary elements; these are the other half of the same
# schema -- decision_gate's own -- and without them "wrong type" would only be caught for
# the fields the policy happens to name, so `"iteration": "1"` would sail through.
# `validate_ledger_record` already enforces several of these when a record is read back;
# this table is what lets the FORM sweep name the field and the expected type instead.
LEDGER_FIELD_TYPES: tuple[tuple[str, type, str], ...] = (
    ("ledger_schema_version", int, "<json integer>"),
    ("sequence", int, "<json integer>"),
    ("iteration", int, "<json integer>"),
    ("run", str, "<json string>"),
    ("phase", str, "<json string>"),
    ("responsible_phase", str, "<json string>"),
    ("source_binding", str, "<json string>"),
    ("recorded_at", str, "<json string>"),
    ("verdict", str, "<json string>"),
    ("evidence", dict, "<json object>"),
    ("prior_open_decision_items", list, "<json array>"),
    ("open_decision_item", bool, "<json boolean>"),
)


_JSON_TYPE_NAMES = {
    "enum": "one of the closed set",
    "boolean": "<json boolean>",
    "declared": "<json boolean>",
    "citations": "<json array>",
    "user_decision": "one of the closed set",
    "policy_source": "<json object>",
}


def collect_form_defects(
    policy: DecisionPolicy, record: Mapping[str, Any]
) -> tuple[GateDefect, ...]:
    """EVERY form defect in one sweep, never just the first.

    `parse_gate_result` stops at the first defect, which is right for a boundary that
    only has to refuse. It is wrong for a bounded repair: a record with three
    independent form errors would consume three attempts against a budget of two and
    exhaust without the agent ever seeing the full list. This function exists for the
    repair path; `parse_gate_result`'s behaviour and its exact reason codes are
    unchanged.

    Every check reads the contract's own declared domain -- the closed record field set
    and `policy.boundary_elements` through `decision_policy.domain_defect` -- so the
    schema stays the single source of truth and this module restates none of it.
    """
    defects: list[GateDefect] = []

    extra = sorted(set(record) - CLOSED_LEDGER_RECORD_FIELDS)
    for name in extra:
        defects.append(_form(
            GATE_INPUT_MALFORMED, name, sorted(CLOSED_LEDGER_RECORD_FIELDS),
            record[name],
            f"{name!r} is outside the closed record field set",
        ))

    for name in REQUIRED_LEDGER_RECORD_FIELDS:
        if name not in record:
            defects.append(_form(
                GATE_INPUT_MALFORMED, name, (), None,
                f"required field {name!r} is absent",
            ))
        elif record[name] is None and name not in _NULLABLE_REQUIRED_FIELDS:
            defects.append(_form(
                GATE_INPUT_MALFORMED, name, (), None,
                f"required field {name!r} is present but null",
            ))

    for name, expected_type, label in LEDGER_FIELD_TYPES:
        if name not in record or record[name] is None:
            continue
        value = record[name]
        # `bool` first: isinstance(True, int) is True, so an int check would accept a
        # boolean where an integer is required -- the same trap `_domain_defect` closes
        # in the other direction.
        wrong = (type(value) is not bool if expected_type is bool
                 else type(value) is bool or not isinstance(value, expected_type))
        if wrong:
            defects.append(_form(
                GATE_INPUT_MALFORMED, name, (label,), value,
                f"{name!r} must be {label}",
            ))

    for element, spec in policy.boundary_elements.items():
        # An element of kind `citations` governs the record's own `citations` field: the
        # contract names the ELEMENT explicit_requirement_conflict but validates the
        # RECORD key `citations`, and without this mapping a wrong-typed citations list
        # would only be caught later, as a judgement, and would never be repairable.
        field = "citations" if spec.kind == "citations" else element
        if field not in record:
            continue
        detail = domain_defect(spec, record[field])
        if detail is None:
            continue
        expected = tuple(spec.values) if spec.values else (
            _JSON_TYPE_NAMES.get(spec.kind, "<see the contract>"),
        )
        defects.append(_form(
            GATE_INPUT_MALFORMED, field, expected, record[field],
            f"boundary element {element!r} {detail}",
        ))

    source = record.get("policy_source")
    if isinstance(source, dict):
        kind = source.get("kind")
        if kind is not None and kind not in policy.policy_source_kinds:
            defects.append(_form(
                GATE_INPUT_MALFORMED, "policy_source.kind",
                policy.policy_source_kinds, kind,
                "policy_source kind is outside the closed set",
            ))
        role = source.get("role")
        if role is not None and role not in policy.policy_source_roles:
            defects.append(_form(
                GATE_INPUT_MALFORMED, "policy_source.role",
                policy.policy_source_roles, role,
                "policy_source role is outside the closed set",
            ))
        locator = source.get("locator")
        if not isinstance(locator, str) or not locator.strip():
            defects.append(_form(
                GATE_INPUT_MALFORMED, "policy_source.locator",
                ("<non-empty text>",), locator,
                "policy_source must carry a non-empty textual locator",
            ))
    elif source is not None:
        defects.append(_form(
            GATE_INPUT_MALFORMED, "policy_source", ("<json object>",), source,
            "policy_source must be an object",
        ))

    return tuple(defects)


# `reason_code` is legitimately JSON null for CLEAR, and `assumption` / `open_item` are
# null whenever there is nothing to record, so a null in these three is not a defect.
# Everything else in the required set is a value the record has to actually carry.
_NULLABLE_REQUIRED_FIELDS: frozenset[str] = frozenset(
    {"reason_code", "assumption", "open_item"}
)
