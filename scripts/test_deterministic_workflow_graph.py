...
class WorkflowGraphTests(unittest.TestCase):
    ...
    def run_graph(self, results, phases=("ANALYSIS",), **kwargs):
        ...
        return build_graph(adapter).invoke(state, config={"recursion_limit": 300}), adapter
    ...
    def test_decision_block_states_override_quality_without_budget_consumption(self):
        ...
        for decision in ("NEEDS_INPUT","CONFLICT"):
            ...
            out=build_graph(adapter).invoke(state)
    ...
    def test_compiled_graph_dedupes_replayed_settlement_event(self):
        ...
        graph=build_graph(adapter,checkpointer=saver,interrupt_before=["APPLY_RESULT"])
    ...
    def test_memory_checkpoint_resumes_prepared_intent_without_duplicate(self):
        ...
        graph=build_graph(adapter,checkpointer=saver,interrupt_after=["PREPARE_INTENT"]); config={"configurable":{"thread_id":"resume"},"recursion_limit":100}
