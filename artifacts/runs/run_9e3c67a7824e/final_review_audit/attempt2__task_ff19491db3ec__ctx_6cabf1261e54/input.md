OS-40 PR#28 — FINAL ADVERSARIAL REVIEW (attempt 2)
...
artifact_contract:
  WRITE: artifacts/runs/run_9e3c67a7824e/FINAL_REVIEW_iteration2.md   (attempt 2)
...
=== RESULT CONTRACT ===
artifacts/runs/run_9e3c67a7824e/FINAL_REVIEW_iteration2.md 에:
# Review Result
RESULT: PASS | FAIL
...
--files-modified 에 artifacts/runs/run_9e3c67a7824e/FINAL_REVIEW_iteration2.md,
body 에 RESULT / REVIEW_VERDICT / DECISION_GATE_STATE / blocking finding 개수.

=== !! ATTEMPT 2 — attempt 1 은 FAIL 이었다 !! ===
FINAL_REVIEW_ITERATIONS = 2 (max-iterations = 5). 이 session 은 attempt 1 의 context 를
상속하지 않는다. 그 결론을 옳다고 가정하지도, 반복하지도 마라.

--- attempt 1 (FAIL, blocking 1건) 과 그 correction (§17 요구 공시) ---
F-001 (G1/CRITICAL, Responsible Phase: bugfix) — crash-safe idempotency 가 opt-in 이라
  기본 외부 실행 경로가 Task/Dispatch 를 중복 생성할 수 있었다.
  `execute_intent_node(..., runtime_state=None)` 이 `_settle_now` 를 선택하고 그 첫 동작이
  `adapter.start(intent)` 였으며, `build_graph`/`execute_state`<REDACTED:foreign_absolute_path> 모두 기본값이 `None` 이었다.
  기존 recovery 테스트가 항상 store 를 주입해 shipped default 를 검증하지 못했다.
  재현: 하나의 stable intent + fresh adapter 2개 → external Task 2건.
  → **BUGFIX phase 를 iteration 4 에서 수정했다:**
    - `resolve_runtime_state` 신설: 명시 인자 → adapter-bound port 순 해석,
      둘이 다르면 `RuntimeStateConflict`, 아무것도 없으면 `IdempotencyPortRequired`.
    - **port 없는 모드(`_settle_now` 분기)를 삭제**했다. 모든 실행이 claim-before-effect
      하나를 지난다.
    - `execute_intent_node` 는 node 생성 시점에, `build_graph` 는 StateGraph 생성 **전에**
      해석하므로 중복 가능한 graph 자체를 만들 수 없다.
    - launcher 는 `--runtime-state` 가 없으면 `$ORCA_OS40_RUNTIME_STATE_DIR`(기본 시스템 temp)
      아래 `<run_id>__<thread_id>.json` 실제 파일 ledger 를 스스로 제공한다.
    - 기존 테스트 호출부 21곳이 `InMemoryRuntimeStateStore` 를 명시적으로 넘기도록 갱신됐다.
    - 부수: launcher CLI 테스트 두 subtest 가 같은 run_id 로 기본 ledger 를 공유해 두 번째가
      첫 번째 settlement 를 복구했다(엔진의 정상 동작). subtest 별 ledger 격리를 추가했다.
  BUGFIX phase gate 는 iteration 4 에서 PASS 했다(0 blocking, 0 non-blocking).
T5a: 이 run 은 specialized(bugfix) 이므로 downstream 집합 D 는 공집합이고 T5a 는 no-op 이다.

--- 현재 수치 (직접 재확인하라) ---
full suite 1828 tests / OK (skipped=6)   [이 작업 시작 시점 baseline 1761 / OK (skipped=6)]
dependency-absent lane 103 tests errors=0
validate_skills 729 checks / verify_package 234 files + archive / graph-doc validator PASSED
source ↔ installed mirror byte-identical
run_workflow.py --demo → COMPLETED / trace 68 / exit 0

--- Coordinator 가 직접 확인한 사실 (재확인해도 좋다) ---
- runtime_state 없는 `execute_intent_node` → `IdempotencyPortRequired` 로 거부
- runtime_state 없는 `build_graph` → **build time** 에 거부
- `FileRuntimeStateStore` 를 같은 파일로 새 객체 2개 + fresh adapter 2회 → 외부 효과 총 1건
  (수정 전에는 2건)
- `--demo` 는 플래그 없이 COMPLETED / trace 68 / exit 0
- `git status --short artifacts/runs/run_0bcf4e7296c9/` → 0줄 (과거 run artifact 무변경)

--- 이번 attempt 가 특히 확인해야 할 것 ---
- attempt 1 이 지적한 "테스트가 shipped default 를 검증하지 않는다" 는 문제가 **다른 형태로
  반복되지 않는가**. 새 default-path 테스트가 정말 runtime_state 를 생략하는가, 아니면
  다른 방식으로 안전한 경로만 밟는가.
- port 없는 모드 삭제가 **과잉**은 아닌가. 정당한 사용 사례를 막지 않는가.
  (launcher 기본 ledger 가 그 자리를 메우는가)
- `IdempotencyPortRequired` 가 build time 에 나는 것이 계약상 적절한가.
  workflow terminal 이 아니라 예외인 것이 정당한가 — Worker 는 "port 부재는 workflow 결과가
  아니라 설정 오류" 라고 근거를 댔다. 동의하는지 스스로 판단하라.
- 기본 ledger 가 시스템 temp 에 쓰이는 것이 durable 하다고 할 수 있는가.
  이것이 알려진 제한사항으로 정직하게 기록되어 있는가.
- M-001 / M-002 / M-003 / M-004 가 훼손되지 않았는가 (attempt 1 이 해소 확인한 것들).