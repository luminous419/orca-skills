OS-40 PR#28 — FINAL ADVERSARIAL REVIEW (attempt 3)
...
artifact_contract:
  WRITE: artifacts/runs/run_9e3c67a7824e/FINAL_REVIEW_iteration3.md   (attempt 3)
...
=== RESULT CONTRACT ===
artifacts/runs/run_9e3c67a7824e/FINAL_REVIEW_iteration3.md 에:
# Review Result
RESULT: PASS | FAIL
...
--files-modified 에 artifacts/runs/run_9e3c67a7824e/FINAL_REVIEW_iteration3.md,
...
상속하지 않는다. 그 결론을 옳다고 가정하지도, 반복하지도 마라.

--- attempt 1 (FAIL, blocking 1) 과 correction ---
F-001 (G1/CRITICAL, bugfix): crash-safe idempotency 가 opt-in 이라 shipped default 경로가
  Task/Dispatch 를 중복 생성했다(하나의 stable intent 에 external Task 2건).
  → BUGFIX iteration 4: `resolve_runtime_state` 신설(명시 인자 → adapter-bound port,
    불일치 시 RuntimeStateConflict, 부재 시 IdempotencyPortRequired), **port 없는 모드
    (`_settle_now`) 삭제**, `build_graph` 는 StateGraph 생성 전에 해석해 중복 가능한 graph 를
    아예 만들 수 없게 함, launcher 는 `--runtime-state` 없으면
    `$ORCA_OS40_RUNTIME_STATE_DIR`(기본 시스템 temp)에 파일 ledger 를 스스로 제공.
    기존 테스트 호출부 21곳이 store 를 명시적으로 넘기도록 갱신됨.

--- attempt 2 (FAIL, blocking 1) 과 correction ---
M-002/G2: public `.compiled` attribute 가 guarded façade 를 벗겨 raw graph 를 돌려줬다
  (facade BLOCKED/0 effects vs `.compiled` COMPLETED/3 effects). `compiled` 는 `__init__` 이
  설정한 실재 attribute 라 deny-by-default `__getattr__` 이 호출되지 않았다.
  → BUGFIX iteration 5: compiled graph 를 façade 에 **전혀 저장하지 않고** module-private
    WeakKeyDictionary(`_COMPILED_GRAPHS`)에 façade 인스턴스를 key 로 보관. `vars(g) == {}`.
    Worker 는 name mangling 과 `_graph` property 도 시도했으나 iteration 3 의 invariant
    테스트가 `dir(facade)` 로 둘 다 잡아내 결국 어떤 member 도 두지 않는 방향을 택했다.
    pin 테스트의 의도는 테스트가 동일 WorkflowState schema 위에 passthrough node 하나짜리
    StateGraph 를 스스로 compile 하는 `_raw_compiled_graph()` 로 보존했다(production 경로 없음).
BUGFIX phase gate 는 iteration 5 에서 PASS 했다(PASS WITH NOTES, 0 blocking).
T5a: specialized(bugfix) 이므로 D 는 공집합이고 T5a 는 no-op 이다.

--- !! 예산 상태 (중요) !! ---
PHASE_ITERATIONS[BUGFIX] = **5 of 5 (소진)**. FINAL_REVIEW_ITERATIONS = 3 of 5.
따라서 이번 attempt 가 Responsible Phase = bugfix 인 blocking finding 을 내면 run 은
STATUS: ESCALATED / MAX_ITERATIONS_REACHED (phase bugfix) 로 종료된다.
**그렇다고 판정을 완화하지 마라** — 정확한 FAIL 은 정당한 결과이고 근거 없는 PASS 는 이 workflow
전체를 무의미하게 만든다. 동시에 취향이나 일반적 개선 제안을 blocking 으로 승격하지도 마라.
기준은 G1-G5 위반 또는 explicit requirement 위반뿐이다.

--- 이번 attempt 가 반드시 판정해야 할 열린 항목 ---
[N-001, BUGFIX iteration 5 reviewer 가 non-blocking 으로 남긴 것]
  `build_graph` 의 docstring(graph.py:194-195)이 **제거된 `.compiled` handle 이 여전히
  도달 가능하다고 잘못 서술**한다: "The underlying compiled graph stays reachable as ...".
  class docstring(75-76)은 올바르게 갱신되었고 코드 동작도 올바르다 — 문서만 stale 하다.
  Coordinator 가 직접 확인했다.
  phase Reviewer 는 Quality Attribute NONE 으로 보아 non-blocking 으로 판정했다.
  **이것이 blocking 인지 아닌지는 이번 attempt 가 스스로 판정한다.** checklist 축 E
  (docs vs behavior)에 정확히 해당하고, 보안 관련 handle 의 존재를 잘못 알리는 문서라는
  점에서 단순 오타와 다르다고 볼 여지도 있다. 다만 BUGFIX 예산이 소진되었으므로 이것을
  blocking 으로 세우면 run 은 ESCALATED 로 끝난다. 그 사실이 판정을 바꾸어서는 안 되지만,
  판정의 근거는 명확히 적어라.

--- 현재 수치 (직접 재확인하라) ---
full suite 1831 tests / OK (skipped=6)   [이 작업 시작 시점 baseline 1761 / OK (skipped=6)]
dependency-absent lane 106 tests errors=0
validate_skills 729 / verify_package 234 + archive / graph-doc validator PASSED
source ↔ installed mirror byte-identical, run_workflow.py --demo COMPLETED/trace 68/exit 0

--- Coordinator 가 직접 확인한 사실 (재확인해도 좋다) ---
- `vars(build_graph(...))` == {} 이고 compiled/_compiled/_graph/graph/pregel/raw/builder/
  validate/copy/__compiled 전부 AttributeError, Pregel 을 돌려주는 member 없음
- unknown-only → BLOCKED/MALFORMED_STATE/0 effects, clean state → COMPLETED/3 effects
- runtime_state 없는 `build_graph` → build time 에 IdempotencyPortRequired
- 같은 파일 ledger + 새 store 객체 + fresh adapter 2회 → 외부 효과 총 1건
- `git status --short artifacts/runs/run_0bcf4e7296c9/` → 0줄