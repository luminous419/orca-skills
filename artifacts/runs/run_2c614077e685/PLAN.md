...
`artifacts/runs/run_2c614077e685/ANALYSIS.md`, which is approved and PASSing at its latest
review recorded in `artifacts/runs/run_2c614077e685/ORCHESTRATOR_LOG.md`.

Reaching that baseline took more than one pass. This run first ran its own internal
analysis → plan → Final Adversarial Review correction cycle; then draft PR #19 received an
external GitHub review, which opened a second, external-review-triggered correction cycle. That
second cycle itself needed several rounds, because Final Adversarial Review kept finding new
issues in each fix. The findings raised across both cycles are **EXT-1** and **EXT-2** (external,
from the PR #19 review) and **R3-1**, **R4-1**, **R5-1**, **R5-2**, **P6-1**, **R6-1** and
**R6-2** (internal); any further round beyond these is recorded in the ledger cited next. One
of them shaped the analysis substantively: the fix for EXT-2 is what introduced hypothesis
**H-5**, carried below as a live alternative to H-4.

For the authoritative, current dispatch/task/iteration provenance of every correction round in
this chain, see `artifacts/runs/run_2c614077e685/ORCHESTRATOR_LOG.md` — this section
intentionally does not duplicate that ledger, because duplicating it here goes stale every time a
new round is dispatched (see findings R4-1, P6-1, R7-1). The reason is structural: a correction
round's text is written *before* that round's own settlement is recorded, so any inline inventory
of this run's own rounds is out of date the moment the next round settles, which always happens
before the next reviewer reads the file. The ledger is appended to as rounds settle and is
filterable by `phase` / `iteration` / `round_kind`, so it is the one account of this chain that is
correct whenever it is read. Naming it here instead of copying it is a deliberate choice, not an
omission. (The run's earlier, pre-external phase-gate dispatches and Final Review attempts sit in
that same ledger and are not part of this chain.) ANALYSIS's
**DEMONSTRATED / HYPOTHESIS** evidence tiers are binding: the Verdict rests on DEMONSTRATED
findings only; H-4 (the affirmative-only search contract), H-5 (a capability weakness specific
to the negative-space defect class), H-1 (anchoring) and H-2 (first-PASS stopping rule) are
hypotheses and are never stated as settled causes. H-4 names the **most directly editable**
contract surface of the four and therefore motivates a P1 intervention — but it is explicitly
**not** evidentially ranked above **H-5**, which predicts the same observations on this corpus and
is not ruled out; no controlled comparison across reviewer models or agent assignments was ever
run, so the two are indistinguishable here. That intervention closes a *demonstrated contract gap*
while its *causal effectiveness* must be **measured** against a seeded-defect fixture that varies
both the checklist **and** the reviewer model / agent assignment, not assumed. H-1 and H-2 drive
only lower-priority, **validate-before-changing** backlog items.
...
*blocking* criteria. ANALYSIS F2 measures a **0/4 dispute/withdrawal rate** and a **4/4
accepted-correction rate** — every internal blocking finding that entered the lifecycle was
accepted and acted on, and none was contested. Those figures measure *acceptance, not
independently-adjudicated correctness*: no blocking false-positive rate exists for this gate and
none may be inferred from them. What the corpus therefore shows is **no observed evidence of a
noise problem**, and on that basis `reviews/common.md`'s `Not Blocking by Default` minimalism is
left alone. Every proposal below adds **search depth or auditability**, never new blocking
criteria (see Risk R-A).
...
Orca's Final Adversarial Review is an **uncontested but shallow** gate — every blocking finding it
issued was accepted and none was ever disputed, but its precision has never been adjudicated
(§6.1), so "uncontested" is the strongest word the evidence supports. On the five runs where an
...
of its own, 4 of which entered the lifecycle, with a 0/4 dispute/withdrawal rate and a 4/4
accepted-correction rate** — including two `SKILL.md` self-contradictions the external channel
never found. Every internal finding cited `file:line` evidence and every run re-ran the validators
itself rather than trusting the Worker's numbers.

**What those two rates measure, and what they do not.** They record what the lifecycle *did* with
each finding — nothing was contested, everything was acted on — not whether each finding was
*true*. Acceptance is not adjudication: none of the 4 has been independently adjudicated against
explicit contract or repository behaviour, and the external channel does not corroborate them
either (channel overlap is zero in both directions, §3.2). **No blocking false-positive rate is
reported anywhere in this report, and none should be inferred**; `0/4 disputed` is equally
consistent with four true positives and with findings that were simply never contested. Every
downstream argument and guardrail below uses the renamed metrics, never a precision claim.
...
**H-4** — an inference rather than an established cause: no Final Review input spec or search
procedure is retained for any run, and model / per-attempt search variance is not excluded.
"The contract permitted stopping" is demonstrated; "the contract caused stopping" is not.

**A second hypothesis predicts exactly the same observations and is not ruled out.** **H-5** — a
systematic capability weakness specific to *this defect class* (counterexample search over
negative-space / boundary / losing-branch paths). Reviewer capability is demonstrably **non-zero
in general**: the same channel produced 4 accepted, concretely-evidenced blocking findings in the
same runs. That says nothing about capability for the missed class specifically — strength at
reading contract self-contradictions is entirely compatible with a systematic weakness at
counterexample search. **No controlled comparison across reviewer models or agent assignments on
the same defects was ever run**, so H-4 and H-5 are **evidentially co-equal on this corpus and
cannot be separated by it**. H-4 is scoped first because it names the most directly editable
contract surface — explicitly **not** because it is better supported. (Visibility and scope *are*
ruled out below by the control checks; capability is not.)

Separating H-4 from H-5 requires the controlled seeded-fixture comparison this report proposes
rather than performs — varying the checklist **and** the reviewer model / agent assignment — which
is why the P1 intervention H-4 motivates is scoped as an experiment whose recall effect must be
**measured**, not as a proven fix. A fixture that varies only the checklist cannot attribute any
improvement, or its absence, to either hypothesis.
...
search contract), H-5 (a defect-class-specific capability weakness), H-1 (anchoring) and H-2
(first-PASS stopping rule) — is structurally unanswerable on the current record, and will remain
so for the next OS-21 unless the audit contract is fixed first. For H-4 vs H-5 specifically,
retaining the input spec is necessary but not sufficient: each attempt's reviewer model / agent
assignment must be recorded alongside it, or the two stay indistinguishable even in future runs.
...
demonstrated contract gaps: the missing falsification search contract (whose recall effect rests
on H-4, is contested by the co-equal H-5, and must be measured in a design that can separate them)
and the missing input/verdict audit contract (a demonstrated defect needing no causal premise). Two further hypothesis-validation tickets are explicitly gated behind
...
EXTERNAL -> INTERNAL (complementarity)
  ...
  internal blocking findings disputed or withdrawn             0     dispute/withdrawal rate  = 0 / 4
  internal blocking findings accepted and corrected            4     accepted-correction rate = 4 / 4
  internal non-blocking findings issued                        3     (all in PR #12; concretely
                                                                     cited, classified non-blocking,
                                                                     never adjudicated)
...
The dispute/withdrawal and accepted-correction rows measure **acceptance, not correctness**:
none of the 4 was independently adjudicated against contract or repository behaviour, and — as
the next paragraph records — the external channel does not corroborate them either. No blocking
false-positive rate is derived from them here or anywhere below.
...
here. Nor is H-4 the only reading: a capability weakness specific to this defect class (**H-5**,
§5.1) predicts the same confirmation-only record and is not ruled out by anything in this
section.
...
FR blessed. **No miss is attributable to missing evidence, artifact invisibility, or out-of-scope
findings.** Those two control checks speak to *visibility* and *scope* only. They say nothing
about reviewer **capability**, which is therefore **not** ruled out here (**H-5**, §5.1). What the
misses *are* attributable to is the open question, and this corpus supports two candidate answers
equally: the contract gap above (**H-4**) and a capability weakness specific to this defect class
(**H-5**). The record is consistent with either and establishes neither; no controlled comparison
across reviewer models or agent assignments was run that could separate them.
...
HYPOTHESIS H-4** — the target of the report's highest-priority intervention because it names the
most directly editable contract surface, **not** because it outranks **H-5** (a capability
weakness specific to the negative-space defect class), with which it is evidentially co-equal and
indistinguishable on this corpus (§5.1). Its effect must therefore be measured rather than
assumed, by a comparison that varies reviewer model / agent assignment as well as the checklist.
...
|---|---|---|---|---|
| **RC-1** | **No falsification obligation in the search contract.** All nine A–I axes are topic labels, read as *is X consistent?* — satisfiable by one confirming instance. §17 states no falsification, enumeration, negative-path, or equality-case obligation, there or anywhere else. The one mandatory global gate is also the only reviewer role with **no policy artifact** — `orca-worker-reviewer-orchestration/reviews/` contains `analysis, plan, design, implementation, test, bugfix, refactoring, common` and **no `final_review.md`**; §17's entire normative content is the A–I block plus one paragraph. | **Contract gap: DEMONSTRATED. Causal link to the misses: HYPOTHESIS H-4 — co-equal with H-5, not evidentially ranked above it** | **DEMONSTRATED:** F3 — 5/5 misses negative-space, each verified present at the PASSed head by `git show`; evidence-availability 5/5, in-scope 5/5; A–I are topic labels stating no falsification/search-depth obligation (`SKILL.md:1698-1890`); `ls orca-worker-reviewer-orchestration/reviews/` shows no `final_review.md`. **NOT DEMONSTRATED:** that this omission is what made the reviewers stop — no input spec or search procedure is retained for any run, model / per-attempt search variance is not excluded, and a defect-class-specific capability weakness (**H-5**, §5.1) is neither excluded nor distinguishable from H-4 on this corpus | Candidate mechanism for **M1, M2, M3, M4, M5** (H-4), competing on equal evidential footing with **H-5**. **Neither is asserted as their proven cause.** |
| **RC-2** | **The gate is unauditable and its verdict unreproducible.** §9 defines no artifact contract for the Final Review *input spec* nor for each *attempt's* report. The dispatch-layer `agent_prompt_blocked` size limit is undocumented and was handled by silently shrinking the spec ~6x. | **DEMONSTRATED** | F5 / §4.4: PR #18's three verdicts on one head; accepted PASS's input **and** report both unretained; `FINAL_RESULT.md:159` | Why the PASS on `0287271` cannot be root-caused; why H-4, H-5, H-1 and H-2 are all structurally unanswerable on this record |
...
### 5.1 Candidate causes assessed — what is ruled out, and what is not

Three of the ticket's candidates are ruled out by the control checks. **Model capability is not
one of them**: it is carried below as hypothesis **H-5**, evidentially co-equal with H-4.

| Candidate (from the ticket's list) | Assessment | Evidence |
|---|---|---|
| Model capability / agent assignment | **HYPOTHESIS (H-5) — NOT ruled out; competes with H-4 and is not evidentially distinguishable from it on this corpus** | Capability is demonstrably non-zero *in general*: the same reviewer channel produced 4 accepted, well-evidenced MAJORs in the same runs, including two `SKILL.md` contradictions the external reviewer never found. That says nothing about capability for the *missed* class — strength at contract-contradiction reading is compatible with a systematic weakness at counterexample search over negative-space / boundary / losing-branch paths. **No controlled comparison across reviewer models or agent assignments on the same defects was run**, so H-5 cannot be separated from H-4 (§4.2) |
...
| Repo/diff inspection **depth** | **Demonstrated as the failing dimension → RC-1 (mechanism = H-4 or H-5; not separable here)** | All 5 misses lie inside files the FR listed as inspected; *why* each search stopped there is not in the record |
| Test-evidence inspection | **Supported → RC-1 (mechanism = H-4 or H-5; not separable here)** | In M5 the *test suite itself encoded the defect*; retained FR reports read "773 tests OK" as evidence of correctness rather than asking which tests bless which behaviour |
...
**H-3 (weak, n=2).** PR #16 spent 3 attempts with two accepted FAILs between them; PR #17 — the only
externally-clean PR — ran 2 attempts with an accepted FAIL between them. The two PRs where the gate
...
ACCEPTANCE OF THE GATE'S OWN FINDINGS (the gate's counter-evidence)
  internal blocking findings issued                                6
  entered the lifecycle                                            4
  disputed or withdrawn                                            0
  dispute/withdrawal rate                                          0 / 4  =   0%
  accepted and corrected as issued                                 4
  accepted-correction rate                                         4 / 4  = 100%
  internal non-blocking findings, none contested                   3
...
CONTRACT COVERAGE
  PRs in the §17 era with a recoverable record of the gate running 6 / 8  (#13, #15 unrecoverable)
  fresh FR re-run after an EXTERNAL correction                     2 / 6  =  33%
  Final Reviews observed at risk=LOW or MEDIUM                     0
```

**What the acceptance block measures, and what it does not.** The dispute/withdrawal and
accepted-correction rates record what the lifecycle *did* with each finding, not whether each
finding was true. Acceptance is not adjudication — an agent can implement a requested correction
even when the finding rested on an incorrect reading of the contract. **No independent
adjudication of these 4 findings against explicit contract or repository behaviour was performed**
(not in the original runs, not in ANALYSIS, not here), and the external channel does not
corroborate them: channel overlap is zero in both directions and at least two of the four are
findings the external reviewer never raised. **No blocking false-positive rate is reported in this
section or anywhere else in this report, and none should be inferred from these rows** — `0/4
disputed` is equally consistent with four true positives and with findings that were simply never
contested. What *is* demonstrated is that the corpus shows **no observed evidence of a noise
problem**; a precision guarantee is a separate claim requiring adjudication work no phase of this
ticket performed.
...
- **Independent validation.** 5/5 runs re-ran `validate_skills.py`, `unittest discover`,
  ...
  numbers.
- **No observed noise problem.** Dispute/withdrawal rate 0/4; accepted-correction rate 4/4 — every
  blocking finding the gate issued was accepted and acted on and none was contested. This measures
  *acceptance*, not adjudicated precision (§6.1), so it is evidence of **no observed noise**, not
  of a proven absence of incorrect findings. On that evidence `reviews/common.md`'s
  `Not Blocking by Default` list and four-tier decision priority are left alone as functioning
  properties.
...
- **Complementary coverage.** The gate found two `SKILL.md` self-contradictions (#16 R1 attempt 1
  ...
  *lose* accepted, concretely-evidenced defect detection the external channel did not supply.
...
- The gate **records confirmations, never falsification attempts** — and §17 never asks it to.
  ...
  either way — and whether they searched but could not find this defect class is H-5, equally
  unrecorded and equally consistent with the same artifacts.
...
| C7 | A fresh Final Review re-runs after correction | **HOLDS in-run; FAILS after external correction** | #16 attempt 3, #12 attempt 2 vs. the 2/6 post-external re-run rate in §6.1 (4 of 6 corrections drew no fresh Final Review) → RC-4 |
...
a re-investigation):* `orca-worker-reviewer-orchestration/reviews/` contains 8 files (7 phase files + `common.md`)
and **no** `final_review.md`; `SKILL.md:1301-1302` carries rule 6 and `:1310` carries
...
**Why not C (INEFFECTIVE).** In the same corpus the gate independently produced **4 blocking
findings that were accepted and acted on, none disputed or withdrawn (0/4 dispute/withdrawal,
4/4 accepted-correction — acceptance, not adjudicated truth, §6.1)**, all with concrete
...
defect class**. Deleting or weakening it would lose accepted, concretely-evidenced detections.
Four of ten contract claims fully hold (C1, C2, C4, C6-mechanical) and two more hold partially
(C5, C7) — §7.

**Therefore.** The gate is **uncontested but shallow, and unauditable at its decision point.**
(*Uncontested*, not *precise*: every finding it issued was accepted and none disputed, but none
was independently adjudicated, so precision is unmeasured — §6.1.) Its failure is a **recall
failure in one identifiable defect class** (negative-space paths),
...
and so is the contract gap that is **one of two co-equal candidates** for producing it: §17
imposes no falsification or search-depth obligation (RC-1). That this gap is *what caused* the
misses is **H-4** and is not carried by the verdict. Scope and visibility are ruled out by the control
checks whatever the true mechanism turns out to be; **capability is not** — a weakness specific to
this defect class (**H-5**) is co-equal with H-4 on this corpus and is likewise not carried by the
verdict. The verdict holds under either mechanism, which is exactly why it does not need to choose
between them.
...
0 / 4   internal blocking findings disputed or withdrawn              §6.1, F2
4 / 4   internal blocking findings accepted and corrected as issued   §6.1, F2
          ^ both measure ACCEPTANCE, not adjudicated correctness
...
*cause* of the misses), H-5 (a capability weakness specific to the negative-space defect class),
H-1 (anchoring) and H-2 (first-PASS stopping rule). All four are consistent with the evidence;
none is established, and H-4 is **not** ranked above H-5 anywhere in this report. The verdict does
not change if any of them turns out false — it rests on the measured recall, the archetype, the
contract text, and the control checks, all of which hold whatever mechanism produced the misses.
...
it closes is demonstrated and is the most directly editable surface in the contract — **not**
because its causal premise outranks the alternative: that premise is **H-4**, which is
evidentially co-equal with **H-5** (a capability weakness specific to this defect class) on this
corpus, and its effect must be **measured, not assumed** (its expected impact is stated below as a
hypothesis to be tested, with a measurement designed to separate H-4 from H-5). I-6 and I-7, whose
hypotheses would justify a *lifecycle* change, are additionally marked *validate before changing
the lifecycle*.
...
`Not Blocking by Default` list or its four-tier decision priority. The guardrail is the
**0/4 dispute/withdrawal rate** recorded in §6.1 — which measures acceptance, not adjudicated
precision, so no proposal may be justified or defended by appeal to a proven absence of incorrect
findings. Any proposal that would push internal blocking findings toward being disputed or
withdrawn is out of bounds by construction.
...
could confirm it, nor excludes per-attempt search variance. **Nor does it exclude H-5** — if the
reviewers were systematically weak at counterexample search over negative-space paths, an
obligation to perform that search may not by itself produce the counterexamples. H-4 and H-5 are
co-equal on this corpus (§5.1), so this proposal's recall effect is a **claim to be tested**, not
an established one — and it is scoped first because it names the most editable surface, not
because it is better supported. What it *does* close regardless of which hypothesis holds is a
real contract gap.

**Mandatory pairing (binding on the follow-up ticket).** The change must be paired with the
controlled measurement that can **separate H-4 from H-5** and size its real effect: the
seeded-defect fixture of **I-5** reviewed **with and without** the falsification obligation **and
across at least two reviewer models / agent assignments**, scored against a known answer key.
Varying only the checklist cannot attribute any recall improvement — or its absence — to either
hypothesis, so the two-factor design is not optional. Because the fixture carries an answer key,
it is also the first context in which a genuine **false-positive rate** becomes measurable at all;
that is a baseline to establish there, not a figure this report already has. Against real runs the
guardrail remains the field-measurable **dispute/withdrawal rate**, reported alongside recall. The
ticket's success criterion is that measurement, not the presence of the new text, and its
description must state H-4 as a hypothesis co-equal with H-5, rather than present the change as
addressing a proven cause.
...
**Regression risk — MEDIUM, and the highest-risk item in this backlog.**
 - *Noise.* Requiring more searching could tempt reviewers into promoting speculative concerns to
   blocking, degrading the 0/4 dispute/withdrawal rate the corpus records. **Mitigation (mandatory in the
...
in this ticket (H-4, H-5, H-1, H-2) is structurally unanswerable, and PR #18's accepted PASS
cannot be root-caused **now or ever**. I-2 is the **precondition for validating H-4, H-5, H-1 and
H-2 at all** — retained input specs are exactly what a future run would need to tell "the reviewer
searched only affirmatively" from "the reviewer searched and missed" — and the precondition for
measuring whether I-1 worked. **For H-4 vs H-5 specifically, retaining the input spec is necessary
but not sufficient:** each attempt's reviewer model and agent assignment must be recorded
alongside it, or the two hypotheses remain indistinguishable even in future runs. That recording
requirement is part of this item's scope.
...
rendered spec, of each attempt's report, and of each attempt's reviewer model / agent assignment;
one documented operational note on the size limit. No
...
**Proposal.** Two parts under one root cause (validation is not repeatable):
 ...
 2. **Build a seeded-defect evaluation fixture.** A small fixture repository/run where known
    ...
    dispatched against a **known** answer key **across at least two reviewer models / agent
    assignments as well as with and without the falsification obligation**. This is the only way
    to (a) **separate H-4 from H-5** and measure whether I-1 actually improved recall — a
    single-model fixture varying only the checklist cannot do this — (b) run the controlled
    two-attempt experiments that I-6 and I-7 require, and (c) detect defects **both** channels
    currently miss — which today are invisible. The answer key also makes a genuine
    **false-positive rate** measurable for the first time; establishing that baseline is part of
...
**Priority: P2 (enabling — must precede I-6 and I-7, and gates both I-1's effectiveness claim and
any attempt to separate H-4 from H-5).**
...
attempts (R-C). PR #16 already spent 3 attempts, and PR #18 spent 3 dispatches on a single
attempt (§4.4).
...
|---|---|---|---|---|---|
| I-1 | RC-1 affirmative-only search contract | gap DEMONSTRATED / cause **H-4, co-equal with H-5** (measure effect; must separate the two) | **P1** | I-2 (recommended); I-5 for the effect measurement | MEDIUM |
...
|---|---|---|---|---|---|---|
| **OS-22** | Final Review falsification search contract (`reviews/final_review.md`) | RC-1 | gap DEMONSTRATED / cause **H-4, co-equal with H-5** | **P1** | 1 | depends on OS-23 (prompt-size headroom); **effect measurement depends on OS-26**; blocks OS-25 |
...
link to the misses: HYPOTHESIS H-4**, which is **evidentially co-equal with HYPOTHESIS H-5** (a
capability weakness specific to this defect class) and is **not** ranked above it — no controlled
comparison across reviewer models or agent assignments was ever run. H-4 is scoped first because
it names the most directly editable contract surface. This ticket is therefore a P1
**intervention whose effect must be measured**, not the application of a proven fix.*
...
unchanged (the **0/4 dispute/withdrawal rate** recorded in §6.1 must be preserved — a measure of
acceptance, not of proven precision; no argument in this ticket may rest on a false-positive rate,
because none has been measured for the gate).
**Out of scope:** phase lifecycle, Risk, Quality Profile, Agent Profile semantics; T0–T5a.
**Binding:** the ticket description must state H-4 as a hypothesis, must name **H-5** as a
competing hypothesis of equal evidential standing on this corpus, and must not present the change
as addressing a demonstrated cause or as evidentially favoured over the capability explanation.
...
**measured** on the OS-26 seeded fixture against a known answer key, in a **two-factor** design —
with and without the obligation **×** at least two reviewer models / agent assignments — so that
the result can distinguish H-4 from H-5; a single-model measurement does **not** satisfy this
criterion. Report the field-measurable **dispute/withdrawal rate** alongside recall as the
guardrail, plus the fixture-only **false-positive rate** the answer key makes measurable for the
first time (as a newly established baseline, not as a preserved 0%). (2) is what separates H-4
from H-5 and is required before the change may be claimed effective.
...
shrinking; (4) record each attempt's **reviewer model and agent assignment** alongside its input
spec — without this, H-4 and H-5 stay indistinguishable even in future runs.
...
shown, **which model / agent assignment reviewed it**, and what it concluded. Explicitly: **a
future OS-21 can root-cause a PASS**, and can compare attempts across reviewer assignments.
...
dispatch procedure that supports varying **both** the search contract **and** the reviewer model /
agent assignment across runs.
**Acceptance:** the Final Review can be dispatched against a known answer key and a recall number
computed reproducibly; OS-22's effect becomes measurable **in a design capable of separating H-4
from H-5** (two-factor: contract × reviewer assignment); a genuine false-positive rate becomes
measurable against the answer key for the first time; OS-27/OS-28's experiments become
executable.
...
- **No expansion of blocking criteria** in any ticket. F2's **0/4 dispute/withdrawal rate** —
  acceptance, not adjudicated precision — shows no observed noise problem, and
  `reviews/common.md`'s `Not Blocking by Default` minimalism is a working property; trading them
  for a speculative recall gain is the single most likely way to make the gate worse (R-A). Note
  this is a "no evidence of harm" argument, not a proven-precision one.
...
- **No lifecycle change justified by H-1 or H-2** before OS-27/OS-28 step 1 reports, and **no
  claim anywhere that H-4 outranks H-5** — the corpus cannot separate them, and OS-22's
  measurement is the only thing that can.
- **No removal or weakening of the Final Review gate.** It contributes 4 unique accepted,
  concretely-evidenced findings the external channel never raised (§6.1); the verdict is B
...
| Do the gap-demonstrated tickets need one? | **Not in order to be authored.** OS-22, OS-23, OS-24, OS-25 are contract/policy work executable today. **But OS-22's *effect* cannot be established without it:** its causal premise is H-4, which this corpus cannot separate from the competing capability hypothesis H-5 — doing so, and sizing the change's real recall effect, requires the OS-26 fixture in a two-factor design (contract × reviewer model / agent assignment). Authoring is unblocked; the effectiveness claim is not. |
...
**The gap, stated concretely:** (1) no seeded-defect fixture with an answer key, and none capable
of varying reviewer model / agent assignment — which is what leaves H-4 and H-5 indistinguishable;
(2) no retained Final Review input spec and no record of which model / agent reviewed each attempt
(fixed by OS-23, which OS-26 depends on); (3) the evidence base itself is unversioned and two
runs' artifacts are already lost.
...
2. **OS-22 second.** It closes a demonstrated contract gap whose causal premise (H-4) is one of
   **two co-equal candidate causes** — the other being H-5, a defect-class capability weakness —
   and carries the largest *hypothesised* recall effect. Second place reflects editability and
   dependency order, **not** evidential ranking over H-5. That effect is only *measurable* once
   OS-26 exists, and only *attributable* in a design that varies reviewer assignment as well as
   the contract, so it must not be judged by feel or reported as a proven improvement first.
...
| V3 | Verdict rests only on DEMONSTRATED evidence | §8's evidence block cites F1–F5/§4/§6 only (measured recall, the archetype, the §17 contract text, the control checks); **H-4**, **H-5**, H-1 and H-2 are all explicitly excluded and the verdict is stated to be invariant to their truth. The acceptance rows are labelled as measuring acceptance, not adjudicated correctness | **PASS** |
| V4 | Hypothesis tier preserved, never upgraded, and never ranked without evidence | **H-4** appears only as RC-1's causal tier, I-1's expected impact, and OS-22's premise — labelled HYPOTHESIS everywhere, with the demonstrated part (archetype + contract gap) stated separately and the intervention's effect gated behind measurement; **H-5** is carried alongside it at every one of those sites (§4.2, §5 preamble, RC-1, §5.1, §8, §9 preamble, I-1, OS-22) as an evidentially co-equal competitor, and nowhere is H-4 stated to outrank it; H-1 appears only in RC-6/I-6/OS-27, H-2 only in RC-7/I-7/OS-28, H-3 only in §5.2 — each labelled HYPOTHESIS with a stated validation precondition and a "no change ships first" clause | **PASS** |
| V5 | Reproducible identities throughout | PRs #10–#18; SHAs `dfe5eed`, `c6a5503`/`c6a55038`, `27690cc`, `087c836`, `0287271`, `33bff91`, `12c60a4`, `b3a1ff3b`, `5d70aed`, `80f604c`, `2b8853aa`, `581f1f3d`, `3a548c98`, `5f9e9368`, `1b51634f`, `0bf13fb`; run ids `run_bf55f06dd7fc`, `run_e0cdf1afae58`, `run_ec18ea04bc22`, `run_c854db299e7a`, `run_1955ca9863a7`; the three PR #18 Final Review dispatch ids `task_2d0a6f4fc5a4`/`ctx_8251971fb59e`, `task_6b7d7a0cdd95`/`ctx_a2ed3c36e1b9`, `task_d3f49c042d5a`/`ctx_71f59c521292`; findings D-002-R1, FINAL-I1-MAJOR-1, M1–M5, F1–F7, RC-1–RC-8, I-1–I-8. **This run's own correction chain** is identified here by finding id only: **EXT-1**, **EXT-2** (external, PR #19), **R3-1**, **R4-1**, **R5-1**, **R5-2**, **P6-1**, **R6-1**, **R6-2** (internal). This run's own correction-chain dispatch/task/iteration identities are not enumerated here (they are unbounded — new rounds keep being added) — see `artifacts/runs/run_2c614077e685/ORCHESTRATOR_LOG.md`, the authoritative and always-current source, filterable by `phase`/`iteration`/`round_kind`. | **PASS** |
...
| V9 | ANALYSIS internal-consistency spot-checks | `ls reviews/` (8 files = 7 phase files + `common.md`, no `final_review.md`); `SKILL.md:1301-1302,1310` (rule 6 + `REVIEWER_CONTEXT_EXCLUDES`); §17 at `:1698`; ladder at `:1790-1802`; `.gitignore` (3 excluded subpaths only) | **PASS**, with one refinement recorded in §7 (the ladder *does* have a DESIGN rung; RC-3 restated precisely) |
...
runs; the effectiveness metric is the same `external blocking recall` figure in §6.1. The
guardrail on **real runs** is the field-measurable `dispute/withdrawal rate`, which is `0/4` today
and must not degrade — noting that this measures acceptance, not adjudicated precision, so it is
not a proven-0%-false-positive baseline. On the **fixture**, where an answer key exists, a genuine
`false-positive rate` becomes measurable for the first time; that is a baseline to establish
there, not a figure this report already holds. **A recall improvement bought with findings that
get disputed, withdrawn, or shown wrong against the answer key is a failure, not a success.**

## Risks

| ID | Risk | Sev | Mitigation |
|---|---|---|---|
| **R-A** | **Over-correcting into noise.** The instinctive reaction to 0% recall is a longer checklist; `reviews/common.md`'s minimalism and the **0/4 dispute/withdrawal rate** say the opposite. That rate records acceptance, not adjudicated precision (§6.1), so the argument here is "no observed noise problem", never "proven absence of incorrect findings". | **HIGH** | Every proposal is constrained to search depth / auditability. OS-22 carries an explicit hard constraint. The dispute/withdrawal rate is named as the real-run guardrail metric in the Validation Plan, with the fixture's answer-key false-positive rate as a baseline to be established, not preserved. |
...
| **R-C** | **Cost.** Every direction (falsification pass, second independent attempt, post-external re-run) increases Final Review dispatches per run. #16 already spent 3 attempts, #18 spent 3 dispatches on one attempt (§4.4). | MEDIUM | Cost is a first-class evaluation criterion inside OS-22, OS-25, OS-28. OS-28's second-attempt option is gated behind a measured benefit. |
...
| **R-G** | **Hypothesis creep, and premature ranking.** H-4, H-5, H-1 and H-2 are attractive explanations and may get treated as settled during backlog grooming — **H-4 most of all**, because it names the most editable surface and already drives a P1 ticket. Treating it as proven would let OS-22 be reported as a fix for a demonstrated cause and skip its measurement. A second, subtler failure is treating H-4's *scoping priority* as evidential *support* and quietly dropping **H-5**, which this corpus cannot separate from it; that would make a negative measurement look like a failed fix rather than evidence for the capability explanation. For H-1/H-2 the creep would produce an unjustified lifecycle change. | MEDIUM | Tier labels are carried into the ticket descriptions (OS-22, OS-27, OS-28), each with a "must not present as proven cause" instruction; OS-22 additionally must name H-5 as co-equal. OS-27/OS-28 carry a "step 1 must report first" clause; OS-22 carries a mandatory **two-factor** effect-measurement acceptance criterion (contract × reviewer model / agent assignment) against the OS-26 fixture, which is what makes the H-4/H-5 distinction reportable at all. |
...
3. **Every miss is root-caused with evidence**, and each root cause carries an explicit evidence
   ...
   archetype and the contract gap are demonstrated, the causal link is not), carried alongside the
   co-equal competing hypothesis **H-5** (a defect-class capability weakness, §5.1), and M5
   additionally to RC-3 (§4, §5). ✔
4. **The DEMONSTRATED / HYPOTHESIS distinction is preserved end to end.** H-4, H-5, H-1 and H-2
   appear only as hypothesis-tier drivers, never as settled causes — H-4 gates OS-22's
   *effectiveness claim* behind measurement without blocking its authoring, and is nowhere ranked
   above H-5 — and the Verdict is stated to be invariant to all four (V4). Likewise no metric in
   this report is stated as a false-positive rate: §6.1 reports a **dispute/withdrawal rate (0/4)**
   and an **accepted-correction rate (4/4)**, both explicitly measures of acceptance rather than
   adjudicated correctness, and every guardrail downstream uses those terms. ✔
...
10. **The harness/fixture-gap question is answered explicitly** — yes, one gap remains; it blocks
    ...
    W-3). ✔
11. **The report is reproducible** — every claim is traceable to a PR number, commit SHA,
    artifact path, run id, or finding id (V5), and this run's own correction chain is traceable
    through its finding ids — EXT-1, EXT-2 (external, PR #19), R3-1, R4-1, R5-1, R5-2, P6-1,
    R6-1, R6-2 (internal) — plus `artifacts/runs/run_2c614077e685/ORCHESTRATOR_LOG.md`, which is
    the authoritative and always-current record of which dispatch, task, phase and iteration
    produced each of those rounds. This criterion deliberately does **not** require this document
    to enumerate its own correction-chain dispatch ids inline: that list is unbounded and every
    inline copy of it is stale as soon as the next round settles (R4-1, P6-1, R7-1). The
    reproducibility guarantee is unchanged — a reader can still reconstruct every round of this
    chain end to end, via the ledger (filterable by `phase`/`iteration`/`round_kind`) rather than
    via a duplicated inline snapshot. ✔
