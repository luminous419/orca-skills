OS-40 FINAL ADVERSARIAL REVIEW (attempt 3)
...
[CF-7] (출처: REVIEW_TEST_iteration3.md N-009, non-blocking, 후속 우선순위 1위로 기록됨)
  TEST reviewer 가 새로 고안한 mutation 중 하나 — **SKILL §14 mandatory unit-test gate 를
  `phase_gate` 에서 제거해도 34개 test 가 전부 통과한다.** guard 자체는 실재하고 올바르게
  동작하므로 코드 결함이 아니라 **coverage 공백**이다.
  Reviewer 가 이것을 blocking 으로 세우지 않은 근거: Final Reviewer 가 F-002 Required Action 에서
  "범위는 그 4건으로 한정한다" 고 명시했고 Worker 가 그것을 정확히 이행했기 때문이다.
  → 이 항목은 Final Adversarial Review attempt 2 가 blocking 인지 아닌지 **스스로 판정**해야 한다.
    mandatory test gate 는 risk 와 무관하게 적용되는 safety floor 이므로, 그 gate 를 지키는
    guard 에 evidence 가 없다는 것은 F-002 와 같은 종류의 G5 gap 일 수 있다.
...
artifact_contract:
  WRITE: artifacts/runs/run_0bcf4e7296c9/FINAL_REVIEW_iteration3.md   (attempt 3)
...
=== RESULT CONTRACT ===
artifacts/runs/run_0bcf4e7296c9/FINAL_REVIEW_iteration3.md 에:
# Review Result
RESULT: PASS | FAIL
REVIEW_VERDICT: PASS | PASS WITH NOTES | FAIL | BLOCKED
DECISION_GATE_STATE: CLEAR | ASSUMPTION_ALLOWED | NEEDS_INPUT | CONFLICT
...
succeeded), --files-modified 에 artifacts/runs/run_0bcf4e7296c9/FINAL_REVIEW_iteration3.md,
...
FINAL_REVIEW_ITERATIONS = 3 (attempt 3)
max-iterations = 5
...
=== !! ATTEMPT 3 — attempt 1 과 2 는 모두 FAIL 이었다 !! ===
FINAL_REVIEW_ITERATIONS = 3 (max-iterations = 5). 이 session 은 이전 attempt 의 context 를
상속하지 않는다. 이전 결론을 옳다고 가정하지도, 반복하지도 마라.

--- attempt 1 (FAIL, blocking 2건) 과 그 correction ---
F-001 (G1/CRITICAL, implementation): unknown reviewer verdict fail-open. `phase_gate` 가 verdict 를
  무검증 반환하고 `route` 마지막 줄이 catch-all ADVANCE_PHASE 여서 closed vocabulary 밖 값이 PASS 와
  동일 전이를 얻었다. → IMPLEMENTATION iteration 4-5 에서 수정:
  `contracts.validate_event()` 신설(MALFORMED_EVENT/UNKNOWN_EVENT), `phase_gate` 가 {"PASS","FAIL"}
  밖을 BLOCK 으로, `route` 마지막 줄이 `ADVANCE_PHASE if gate=="PASS" else "BLOCK"`, `final_gate`
  일관화. iteration 5 는 checkpoint 복구 경로 guard test 를 추가(phase Reviewer 가 F-003 으로 요구).
F-002 (G5/MAJOR, test): guard 4개를 제거해도 suite 통과. → TEST iteration 3 에서 fail-closed
  test 4건 추가. 4개 guard mutation 이 이제 failures 6/1/1/2 로 검출된다.
T5a round 1: D={TEST} 를 TEST iteration 4 에서 재검증. PASS, 코드 변경 없음.

--- attempt 2 (FAIL, blocking 1건) 과 그 correction ---
F-001 (G2/CRITICAL, implementation): `terminal_node` 의 ESCALATE 분기가
  `correction_queue[correction_index]` 를 bounds check 없이 읽어, correction round 를 모두 소비한 뒤
  final review budget 이 소진되면 uncaught IndexError 로 죽고 계약된 ESCALATED terminal state 가
  생성되지 않았다. checkpointer 사용 시 next=('TERMINAL',)<REDACTED:foreign_absolute_path> 고착으로 resume 불가.
  (AC 3 / AC 8 / AC 10·DESIGN §6 위반. 58,650 scenario fuzz 중 directed sweep 825건에서 4건 발생.)
  → **이 시점에서 run 은 STATUS: ESCALATED / MAX_ITERATIONS_REACHED (phase implementation) 로
    한 번 종료되었다** (PHASE_ITERATIONS[IMPLEMENTATION] 이 max 5 소진). **사용자가 명시적으로
    예산 연장을 승인해** iteration 6 이 열렸다.
  → IMPLEMENTATION iteration 6: `routing.active_correction_phase(state) -> str | None` 순수
    predicate 신설(bounds + membership + non-int/bool index 거부). `terminal_node` :155·:167 양쪽,
    `route` T4 guard(None→BLOCK), `prepare_intent_node`(None→OUT_OF_ORDER_EVENT) 가 모두 사용.
    package 전체에 `correction_queue[` 직접 index 0건. queue 소비 완료 시 reason code 는
    `FINAL_REVIEW_MAX_ITERATIONS_REACHED` 로 확정하고 DESIGN §6(:83, :324)에 반영. mirror 동기화.
    회귀 테스트 2건 추가.
T5a round 2: D={TEST} 를 TEST iteration 5 에서 재검증. PASS. **AC 3 evidence 를 갱신했다**
  (기존 서술이 crash 하던 경로를 검증된 것처럼 기술하고 있었다).

--- 현재 예산 상태 (중요) ---
PHASE_ITERATIONS: ANALYSIS 2/5, PLAN 2/5, DESIGN 1/5, **IMPLEMENTATION 6 (사용자 승인 연장)**,
  **TEST 5/5 (소진)**. FINAL_REVIEW_ITERATIONS = 3/5.
따라서 이번 attempt 가 implementation 또는 test 를 Responsible Phase 로 하는 blocking finding 을
내면 run 은 다시 ESCALATED 로 종료된다. **그렇다고 판정을 완화하지 마라** — 정확한 FAIL 은
정당한 결과이고, 근거 없는 PASS 는 이 workflow 전체를 무의미하게 만든다.
동시에 취향·일반적 개선 제안을 blocking 으로 승격하지도 마라. 기준은 G1-G5 위반 또는
explicit requirement 위반뿐이다.

--- 현재 수치 (직접 재확인하라) ---
full suite 1761 / OK (skipped=6)   [브랜치 작업 전 baseline 1725 / OK (skipped=6)]
targeted 36 / OK,  dependency-absent lane 36 / OK (skipped=20) errors=0
validate_skills 727 checks, verify_package 226 files, graph-doc PASSED, git diff --check 무출력
engine 11개 파일이 설치본 mirror 와 byte-identical

--- 이번 attempt 가 판정해야 할 열린 항목 (attempt 2 는 전부 non-blocking 으로 결론냈다) ---
CF-7 / N-009: SKILL §14 mandatory unit-test gate 를 `phase_gate` 에서 제거해도 test 가 전부
  통과한다(coverage 공백; guard 자체는 동작한다). attempt 2 는 "이번 FAIL 은 CF-7 때문이 아니라
  오직 F-001 때문" 이라고 명시하며 non-blocking 으로 판정했다. 같은 결론에 도달하는지 확인하라.
N-011 / N-015: TEST.md 의 T5a round-1 절이 인용한 mutation 실패 개수가 실측보다 적다
  (기재 4·1 대 실측 5·2). 정성적 주장은 참이고 round 2 가 인용한 수치는 정확하다.
CF-6 이월 4건: N-002 D 계산 중복(OS-40 "중복 transition engine 금지" 와 맞닿음) /
  N-003 SKILL prose 축소 미이행 / N-004 validator 강도 / N-005 _langgraph_ok 중복.