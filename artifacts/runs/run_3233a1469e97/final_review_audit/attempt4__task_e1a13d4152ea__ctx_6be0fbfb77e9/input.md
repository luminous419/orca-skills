...
FINAL_REVIEW_ITERATIONS: 4   /   max-iterations: 5
...
base..HEAD: c264e79..HEAD  (31 commits, unpushed, 80 files, +9050, -0)
...
analysis, plan, design, implementation, test — 전부 요청되었고 전부 phase gate PASS.
  ...
  IMPLEMENTATION 6 iterations, PASS  (사용자가 UD-5로 예산을 5->7로 늘렸다)
  TEST           5 iterations, PASS
...
이 사실을 보고에 기록한다.
이전 attempt 1, 2, 3이 모두 FAIL이었다. 아래를 반드시 읽어라.
...
=== 직전 attempt들의 finding과 해소 ===
**attempt 1 FAIL** FR-1(test) 전이 행렬 값 미고정 -> 16셀 전부 값 고정(C26) + authority edge
  명명 검사(C26a) + sweep에서 4개 key 추가 발견(C27/C28/C29).
  FR-2(impl) 권한이 5토큰 denylist -> 닫힌 allowlist `user_decision_sources`로 교체, denylist는 강등.
**attempt 2 FAIL** FR-3(impl) reason code가 bind한 boundary element와 record 값의 일치 미검증
  -> exact equality 강제 + 10개 code 전수 mismatch 주입 테스트.
  FR-4(impl, CRITICAL) `permitted_states()`가 entry condition을 평가하지 않고 세 상태를 무조건 허용
  -> A3-1 문언을 `entry_conditions`(닫힌 predicate 12종)와 element별 triggering 값으로 옮겨 실제 평가.
  이어 phase Reviewer가 RI3-1(CRITICAL) 발견 — predicate 독립 평가로 조합에서 precedence 붕괴
  -> A4-0이 지명한 두 셀을 `authority_precedence`로 옮기고 양쪽 predicate에 적용.
**attempt 3 FAIL** FR-5(impl, CRITICAL) `permitted_states()`가 필수 필드 없는 user_decision도
  source만 맞으면 권한으로 인정 — `validate_transition()`은 같은 evidence를 거부했다(두 API 반대 판정)
  -> `_user_decision_defect` 공유 helper + parity 테스트(일치뿐 아니라 옳음까지 단언)
     + 결함을 고정하고 있던 테스트 2개 수정.
**TEST revalidation이 계약 결함 3건을 보고(고치지 않음)** TR4-1 role 멤버십이 한쪽에서만 강제,
  TR4-2 ASSUMPTION_ALLOWED 규칙이 48조합 중 6개 불일치, TR4-3 공백이 evidence로 통과.
  IMPLEMENTATION 예산이 소진되어 **사용자에게 물었고 예산 증액을 승인받았다(UD-5)**.
  -> iteration 6에서 셋 다 공유 helper로 해소. **계약은 바뀌지 않았다** —
     세 경우 모두 계약이 이미 옳은 규칙을 담고 있었고 한쪽 API만 읽고 있었다.
     48조합 divergence 0(허용 조합 수는 2로 불변), 9개 다중 위치 개념 109 케이스 divergence 0,
     그리고 **call-closure 테스트**로 앞으로 한쪽 API에만 규칙이 추가되면 실패하게 만들었다.
**TEST 최종 revalidation** 안전망 27/27 CAUGHT, 아홉 유형 sweep이 **이 run에서 처음으로 빈 결과**.
  call-closure 장치를 실제로 돌려 DESIGN 문구가 기제보다 넓음을 발견하고
  의미 있는 절반만 닫았다(RT5-N1로 기록).

**이 attempt는 위 재검증을 대체하지 않고 그 위에 추가로 걸리는 global gate다.
앞선 gate PASS를 옳다고 가정하지 마라.**

=== 이 run에서 반복된 결함 계열 (아홉 유형) ===
...
**전부 "초록인데 아무것도 지키지 않는 검증"의 변형이다.
직전 TEST sweep이 처음으로 빈 결과를 냈지만 그것을 근거로 끝났다고 가정하지 마라.**

=== 이번 attempt에서 특히 확인할 것 ===
- 여덟 수정(FR-1~FR-5, RI3-1, TR4-1~3)이 서로를 약화시키지 않았는가
- **과잉 제한이 아닌가.** 정당한 CLEAR / ASSUMPTION_ALLOWED 경로가 살아 있는가.
  `ASSUMPTION_ALLOWED`를 허용하는 48조합 중 2건이 유지되는가.
  "전부 NEEDS_INPUT"은 티켓이 명시적으로 금지한 잘못된 구현이다.
- DESIGN.md가 실제 코드와 일치하는가 (correction마다 갱신되었다:
  D1-1/D1-2/D2-2/D2-2a~d/D3/D3-2/D3-3/D4-F)
- 알려진 잔여 갭의 기록이 정확한가
...
- RT5-N1: call-closure는 이름을 남기는 helper만 본다. 인라인 `_require`는 통과한다.
  `permitted_states` 쪽은 인라인 0개 단언으로 닫았고, `validate_record` 쪽은 record-shape
  규칙이라 의도적으로 닫지 않았다.
...
전체 diff:      git diff c264e79..HEAD        (80 files, +9050, -0)
커밋:           git log --oneline c264e79..HEAD   (31개)
...
테스트/검증 결과 (직접 재실행해 대조하라):
  ...
  python3 -m unittest discover -s scripts -p 'test_*.py' -> 보고값 1426 tests OK (skipped=6)
