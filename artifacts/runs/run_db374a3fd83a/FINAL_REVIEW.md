...
- **Location:** `scripts/clarification_protocol.py:682-711` (`_effective_decision`); read sites at
  lines 682-683 (`decisions/decision_*/record.json`) and 689-690 (`lineage/*/event.json`); also
...
`/e2e_harness.py` is untracked, is **not** in `.gitignore`, is not tracked on `main`, and is not
