...
| `coordinator_audit/<NNNNNN>/record.json` | one immutable record per Coordinator delivery/quiescence outcome (OS-44): which deliveries were processed, which were acknowledged and after how many attempts, which were refused because they named another Task or Dispatch, which were replays that took no lifecycle action, how far each delivery's settlement got (claimed before the settlement path's first command, settled after state and settlement are reflected and before the acknowledgement), how a restarted process disposed of a redelivery, and what the quiescence self-check concluded immediately before the Coordinator turn ended. It is the only source a restarted Coordinator can rebuild the delivery ledger from, so unlike the two logs beside it a record that cannot be written or read fails closed. Each record is published BEFORE the in-memory progress state it describes is committed, so the row a successor recovers can never be behind the Coordinator's own memory of it. Same durability as `decision_ledger/` — staged, fsynced, published by one `os.rename()`, never edited |
